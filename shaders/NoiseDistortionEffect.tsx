"use client";
import { forwardRef, useMemo, useEffect } from "react";
import { Uniform, WebGLRenderer, WebGLRenderTarget } from "three";
import { Effect, BlendFunction } from "postprocessing";

/**
 * Noise Distortion Effect
 * Adds FBM (Fractal Brownian Motion) noise-based organic distortion
 * This creates the flowing, organic feel that simple sine waves can't achieve
 */
const noiseDistortionShader = /* glsl */ `
uniform float uTime;
uniform float uStrength;
uniform float uScale;

vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec2 mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 permute(vec3 x) { return mod289(((x*34.0)+1.0)*x); }

float snoise(vec2 v) {
    const vec4 C = vec4(0.211324865405187, 0.366025403784439, -0.577350269189626, 0.024390243902439);
    vec2 i = floor(v + dot(v, C.yy));
    vec2 x0 = v - i + dot(i, C.xx);
    vec2 i1 = (x0.x > x0.y) ? vec2(1.0, 0.0) : vec2(0.0, 1.0);
    vec4 x12 = x0.xyxy + C.xxzz;
    x12.xy -= i1;
    i = mod289(i);
    vec3 p = permute(permute(i.y + vec3(0.0, i1.y, 1.0)) + i.x + vec3(0.0, i1.x, 1.0));
    vec3 m = max(0.5 - vec3(dot(x0,x0), dot(x12.xy,x12.xy), dot(x12.zw,x12.zw)), 0.0);
    m = m*m*m*m;
    vec3 x = 2.0 * fract(p * C.www) - 1.0;
    vec3 h = abs(x) - 0.5;
    vec3 ox = floor(x + 0.5);
    vec3 a0 = x - ox;
    m *= 1.79284291400159 - 0.85373472095314 * (a0*a0 + h*h);
    vec3 g;
    g.x = a0.x * x0.x + h.x * x0.y;
    g.yz = a0.yz * x12.xz + h.yz * x12.yw;
    return 130.0 * dot(m, g);
}

float fbm(vec2 p) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
    for(int i = 0; i < 5; i++) {
        value += amplitude * snoise(p * frequency);
        amplitude *= 0.5;
        frequency *= 2.0;
    }
    return value;
}

void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    float time = uTime * 0.3;
    float noiseX = fbm(uv * uScale + time);
    float noiseY = fbm(uv * uScale + time + 100.0);
    
    vec2 distortedUv = uv + vec2(noiseX, noiseY) * uStrength * 0.02;
    outputColor = texture2D(inputBuffer, distortedUv);
}
`;

class NoiseDistortionEffectImpl extends Effect {
    constructor(strength = 1.0, scale = 3.0) {
        super("NoiseDistortionEffect", noiseDistortionShader, {
            blendFunction: BlendFunction.NORMAL,
            uniforms: new Map<string, Uniform>([
                ["uTime", new Uniform(0)],
                ["uStrength", new Uniform(strength)],
                ["uScale", new Uniform(scale)],
            ]),
        });
    }

    update(_r: WebGLRenderer, _i: WebGLRenderTarget, deltaTime: number): void {
        this.uniforms.get("uTime")!.value =
            (this.uniforms.get("uTime")!.value as number) + deltaTime;
    }

    set strength(v: number) {
        this.uniforms.get("uStrength")!.value = v;
    }
    set scale(v: number) {
        this.uniforms.get("uScale")!.value = v;
    }
}

export interface NoiseDistortionEffectProps {
    /** Distortion strength (0-3) @default 1.0 */
    strength?: number;
    /** Noise scale (1-10) @default 3.0 */
    scale?: number;
}

export const NoiseDistortionEffect = forwardRef<
    NoiseDistortionEffectImpl,
    NoiseDistortionEffectProps
>(function NoiseDistortionEffect({ strength = 1.0, scale = 3.0 }, ref) {
    const effect = useMemo(
        () => new NoiseDistortionEffectImpl(strength, scale),
        // eslint-disable-next-line react-hooks/exhaustive-deps
        []
    );
    useEffect(() => {
        effect.strength = strength;
    }, [effect, strength]);
    useEffect(() => {
        effect.scale = scale;
    }, [effect, scale]);
    return <primitive ref={ref} object={effect} dispose={null} />;
});

export { NoiseDistortionEffectImpl };
